...
if ($action == 'export_expenses') {
    ...
    try {
        ...
        header('Content-Disposition: attachment; filename="expenses_' . $month . '_' . $year . '.csv"');

        $output = fopen('php://output', 'w');
        fputcsv($output, ['Date', 'Description', 'Category', 'Payment Method', 'Card', 'Currency', 'Original Amount', 'Amount (AED)', 'Tags', 'Type']);
        ...
        foreach ($expenses as $e) {
            fputcsv($output, [
                ...
                $e['category'],
                $e['payment_method'],
                $e['card_name'] ? ($e['bank_name'] . ' - ' . $e['card_name']) : ($e['payment_method'] == 'Card' ? 'Unknown Card' : 'N/A'),
                $e['currency'] ?: 'AED',
                $e['original_amount'] ?: $e['amount'],
                $e['amount'],
                $e['tags'],
                isset($e['is_fixed']) ? ($e['is_fixed'] ? 'Fixed' : 'Variable') : 'N/A'
            ]);
        }
        fclose($output);
        exit();
    ...
    try {
        ...
        header('Content-Type: text/csv');
        header('Content-Disposition: attachment; filename="income_' . $month . '_' . $year . '.csv"');
        ...
        foreach ($income as $i) {
            fputcsv($output, [
                ...
                $i['amount'],
                $i['currency'] ?: 'AED',
                isset($i['is_recurring']) ? ($i['is_recurring'] ? 'Yes' : 'No') : 'N/A'
            ]);
        }
        fclose($output);
        exit();
    ...
    try {
        ...
        header('Content-Type: text/csv');
        header('Content-Disposition: attachment; filename="sadaqa_' . $month . '_' . $year . '.csv"');
    ...
    try {
        ...
        header('Content-Type: text/csv');
        header('Content-Disposition: attachment; filename="incentives_' . $month . '_' . $year . '.csv"');
    ...
    try {
        ...
        header('Content-Type: text/csv');
        header('Content-Disposition: attachment; filename="interest_' . $month . '_' . $year . '.csv"');
    ...
    try {
        ...
        header('Content-Type: text/csv');
        header('Content-Disposition: attachment; filename="zakath_calculations.csv"');

        $output = fopen('php://output', 'w');
        fputcsv($output, ['Date', 'Cycle Name', 'Cash', 'Gold/Silver', 'Investments', 'Liabilities', 'Total Zakath', 'Status']);
    ...
    try {
        ...
        header('Content-Type: text/csv');
        header('Content-Disposition: attachment; filename="lending_records.csv"');
