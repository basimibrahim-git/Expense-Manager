...
if ($end_date) {
$query .= " AND e.expense_date <= :end"; $params['end']=$end_date; } $query .=" ORDER BY e.expense_date DESC" ; try {
    ...
    fputcsv($output, ['Date', 'Description', 'Category', 'Payment Method', 'Card', 'Currency', 'Original Amount',
    'Amount (AED)', 'Tags', 'Type']);
    ...
    if ($end_date) {
    $query .= " AND income_date <= :end"; $params['end']=$end_date; } $query .=" ORDER BY income_date DESC" ; try {
        $stmt=$pdo->prepare($query);
        $stmt->execute($params);
        ...
        $r['title'],
        abs($r['amount']),
        $r['amount'] < 0 ? 'Payment' : 'Interest' ]); } fclose($output); exit(); } catch (Exception $e) { error_log($e->
            getMessage());
            die(SYSTEM_ERROR_MSG);
            }
            ...
            $output = fopen(PHP_OUTPUT, 'w');
            fputcsv($output, ['Date', 'Cycle Name', 'Cash', 'Gold/Silver', 'Investments', 'Liabilities', 'Total Zakath',
            'Status']);
