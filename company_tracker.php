...
<?php if (!$selected_year): ?>
    ...
    <div id="addYearDiv" class="d-none mt-4 text-center glass-panel p-3 mw-400px mx-auto">
        ...
        <form class="d-flex gap-2 justify-content-center" onsubmit="event.preventDefault(); window.location.href='?year=' + this.customYear.value">
...
<?php else: ?>
    ...
    foreach ($all_incentives as $row) {
        $m = $row['month'];
        if (!isset($monthly_data[$m])) $monthly_data[$m] = [];
        $monthly_data[$m][] = $row;
        
        if (!isset($monthly_totals[$m])) $monthly_totals[$m] = 0;
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                ...
                <div class="small text-muted fw-bold text-success">Total: AED <?php echo number_format($year_total, 0); ?></div>
            </div>
        </div>
        
        <!-- Year Nav -->
        <div class="btn-group btn-group-sm">
            <a href="?year=<?php echo $year - 1; ?>" class="btn btn-outline-secondary"><i class="fa-solid fa-chevron-left"></i></a>
            <button type="button" class="btn btn-light fw-bold px-3"><?php echo $year; ?></button>
            <a href="?year=<?php echo $year + 1; ?>" class="btn btn-outline-secondary"><i class="fa-solid fa-chevron-right"></i></a>
        </div>
    </div>
                ...
                <div class="modal-body text-center pt-0">
                    ...
                    <form method="POST">
                        ...
                        <div class="form-floating mb-2">
                            <input type="number" step="0.01" name="amount" class="form-control text-center fw-bold" id="amountInput" placeholder="0.00" required>
        ...
        if (items.length > 0) {
            items.forEach(item => {
                ...
                div.innerHTML = `
                    ...
                    <button type="button" class="btn btn-outline-danger btn-sm border-0" onclick="confirmDelete(${item.id})">
    ...
    function confirmDelete(id) {
        // Hide Manage Modal temporarily or just stack? Bootstrap supports stacked.
        // But to be clean, let's just open the Delete Modal. 
        // Note: Stacked modals can be tricky with backdrops, but let's try.
...
<style>
    .transform-hover:hover { transform: translateY(-3px); transition: transform 0.2s; }
</style>
